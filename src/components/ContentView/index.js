...
const ContentView = createWithRemoteLoader({
  ...
  forwardRef(({ className, remoteModules, data, groupCode, objectCode, referenceContents, isInline, plugins }, ref) => {
    const [usePreset, InfoPage, Descriptions] = remoteModules;
    const { apis } = usePreset();

    const renderObjectContent = ({ ref, data, title, groupCode, objectCode }) => {
      return (
        <Fetch
    ...
    return <InfoPage className={className}>{renderObjectContent({ ref, data, groupCode, objectCode })}</InfoPage>;
  })
);
